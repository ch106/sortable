...
Jh.fn = function (a) {
    return a = {
        ...
        _createActionButton: function () {
            var b = $("<a class='button b' href='#' >选择人员</a>"),
                c = $("<a class='button b' href='#' >保存</a>");
                ...
                $.fallr("show", {
                    buttons: {
                        button1: {
                            text: "确定",
                            onclick: function () {
                                $("#portal tbody").append($("<tr class='" + Jh.Config._groupWrapperClass + " ui-sortable' unselectable='on' style='user-select: none;'></tr>"));
                                var b = $("#portal tbody tr:last-child");
                                //tr class="groupWrapper ui-sortable"
                                //获取到json，处理
                                json.forEach((val) => {
                                    var e = val.name,
                                        d = val.id;
                                ...
                                Jh.Portal._bindEvent(b)
                                $.fallr("hide")
                            }
                        },
                        button2: { text: "取消" }
                    },
                    content: '<form>' + json + '</form>',
                    icon: "add",
                    position: "center"
                })
                /* $.fallr("show", {
...
Jh.Portal = function (a) {
    var b = "<tr class='" + Jh.Config._groupWrapperClass + "'></tr>",
        c = "<tr class='" + Jh.Config._groupWrapperClass + "'></tr>",
        e = "<tr class='" + Jh.Config._groupWrapperClass + "'></tr>",
        d = "<td id='{key}' class='" + Jh.Config._groupItemClass + "'></td>",
        f = "<div class='" + Jh.Config._groupItemHead + "'><h3>{name}</h3></div>",
        i = "<div class='" + Jh.Config._groupItemContent + "'></td>";
    ...
    //     f = "<div class='" + Jh.Config._groupItemHead + "'><h3>{name}</h3></div>", i = "<div class='" + Jh.Config._groupItemContent + "'/>";
    return a = {
        init: function (b) {
            a._create();
            a._bindData(b);
        ...
        _createPortal: function (b, c) {
            var d;
            switch (b) {
                case "appL": d = a._elements.m_l; break;
                case "appM": d = a._elements.m_m; break;
